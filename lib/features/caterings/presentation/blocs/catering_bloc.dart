...
class CateringBloc extends Bloc<CateringEvent, CateringState> {
  CateringBloc({required this.cateringRepository}) : super(const CateringState()) {
  ...
  Future<void> _onCateringSubscriptionRequested(
    ...
    try {
      ...
      final caterings = await cateringRepository.getCaterings(offset: state.caterings.length);
      caterings.isEmpty
          ? emit(state.copyWith(
        hasReachedMax: true,
        status: CateringOverviewStatus.success,
